...
- **代理运行与节点管理增强**：支持 Root Tun（无需 VPN，但需要 Root 权限）、单节点与代理组测速、首页当前节点测速、节点排序动画、节点标签与图标/旗帜显示、节点选择持久化，并优化代理启动流程和部分 bridge 实现。
- **ACG 首页与一言体验**：ACG 首页支持壁纸、侧栏、启动按钮、运行信息与一言展示；一言可分别启用 API 与用户自定义 JSON 列表，支持手动刷新、每日自动刷新、刷新加载动画和自定义/接口混合选取。
...
v0.5.3 主要继续完善日常使用中的 MD3 体验，重点集中在个性化、配置编辑、代理模式切换、多语言与文档说明：

- **主题与个性化设置**：新增 ACG 壁纸取色模式、壁纸重置入口、页面缩放滑块触感反馈，并改进不同 Android/MIUI 环境下跳转应用电池优化设置的兼容性。
- **每日一言流程**：完善 ACG 每日一言设置，支持 API/自定义来源开关、本地化自定义 JSON 模板、一言历史记录、刷新状态反馈，并补充独立格式说明文档。
- **配置编辑器体验**：为原生编辑器加入轻量 JSON/YAML 语法高亮，补齐编辑器标题、操作、Toast、诊断信息和未保存修改提示等多语言文案。
- **代理页面交互**：在代理页面加入 Rule/Global/Direct 模式切换，优化分页/分组交互，并调整节点测速与节点选择流程。
- **多语言扩展**：新增繁体中文语言支持，扩展设置页、编辑器、覆写表单、SubStore 错误提示和通用组件文案翻译。
- **稳定性与兼容性清理**：加强压缩包解压路径检查、SubStore 错误提示、Toast 弹窗默认标题处理，并同步 Lite 设置页语言选项。

## 设计与资料参考

当前实现继续收敛旧 UI 边界，普通页面优先使用 Jetpack Compose Material 3 组件与项目内 MD3 组件；Miuix 仅作为少量遗留/兼容边界保留，后续会继续减少直接依赖。

v0.5.3 同时继续参考 `chat-export-2026-04-28_22-27-47.md` 中关于莫奈动态取色的设置建议：以壁纸/系统源色为灵感、以可读性为底线、以 Material 色彩角色为结构，并为用户保留适度控制权。实现侧没有改动代理内核，主要集中在 UI、设置、文档、多语言与交互细节：

- ACG 首页与设置：在保留壁纸、侧栏、启动按钮与运行信息布局的基础上，补充一言刷新、当前节点测速、壁纸重置和刷新状态反馈，并继续随全局主题令牌做克制协调。
